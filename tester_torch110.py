...
for img_path in sample_paths:
    ...
    model_list = get_model_list()   #models.list_models(module=models)
    ...
    Layer_Latencies[img_path] = {}
    # print("op_executor.operations: ",op_executor.operations)
    # x_test = torch.from_numpy(np.array(x))
    ...
    for idx, layer in enumerate(layers):
        try:
            ...
            local_latency_array = np.array(local_latency_array,dtype=np.float32)
            Layer_Latencies[img_path][idx] = np.sum(local_latency_array)/100.0
        except Exception as e:
            print(f'Layer {idx} failed: {e}')
            break

    # print("x.shape: ", x.shape)
    # print("Layer_Latencies: ", Layer_Latencies)
